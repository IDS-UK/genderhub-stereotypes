...
The theme uses is based on [Foundation Press](https://github.com/olefredrik/foundationpress/) which integrates [Foundation 5](http://foundation.zurb.com/).

The project uses Sass, found in the `scss` directory, which is compiled to CSS and reloaded to the browser automatically by Grunt. When developing navigate to the theme root and run:

```
$ grunt
```

Start MAMP and navigate to `http://localhost:8888`

## Debugging

Log Using: `error_log('Message);`

Tail log using: 'tail -f wp-content/debug.log'
